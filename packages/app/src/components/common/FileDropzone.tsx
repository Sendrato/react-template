...
import CloseIconModal from '@components/icons/CloseIconModal';
...
const FileDropzone = ({
  ...
  const formats = acceptKeys
    .map((key) => {
      return accept[key].map((item) => item.toUpperCase().substring(1, item.length));
    })
    .flat()
    .join(', ');
  ...
  return file || base64 ? (
    <UploadedWrapper>
      ...
      {acceptedFiles?.[0] ? (
        <Box width="100%">
          ...
          <FileSize variant="body1">{`${getFileSize(acceptedFiles[0].size)} • Complete`}</FileSize>
        </Box>
      ) : null}
      <CloseIconModal
    ...
    <DropzoneWrapper {...getRootProps({ className: 'dropzone' })}>
      ...
      <TypographyStyled variant="body2" color="rgba(0, 0, 0, 0.6)" fontWeight={400}>
